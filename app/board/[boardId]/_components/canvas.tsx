...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const insertPath = useMutation(
    ({ storage, self, setMyPresence }) => {
      ...
      liveLayers.set(
        id,
        new LiveObject(penPointsToPathLayer(pencilDraft, lastUsedColour))
      );
      const liveLayerIds = storage.get("layerIds");
  ...
  const layerIdsToColourSelection = useMemo(() => {
    ...
    for (const user of selections) {
      ...
      for (const layerId of selection) {
        layerIdsToColourSelection[layerId] = connectionIdToColour(connectionId);
      }
    }
        ...
        >
          {layerIds.map((layerId) => (
            <LayerPreview
              ...
              selectionColour={layerIdsToColourSelection[layerId]}
            />
          ))}
